document.addEventListener("DOMContentLoaded", function () {
    ...
    function captureTrigger(e) {
        ...
        if (/^popup[_\-]/i.test(normalized) || normalized.includes('popup_form')) return;
    ...
    function fallbackSourceFromDOM(button, form) {
        ...
        if (form.closest('.popup-overlay')) section = 'popup';
            ...
            else if (formParent.classList.contains('order__section')) { section = 'contact'; detail = 'primary'; }
            ...
            } else if (formParent.classList.contains('results__section')) section = 'portfolio';
            else if (formParent.dataset.theme === 'dark') { section = 'contact'; detail = 'secondary'; }
            else if (formParent.dataset.theme === 'light') { section = 'footer'; detail = 'primary'; }
        }

        if (form.closest('footer') || (form.closest('[data-theme="light"]') && form.closest('[data-theme="light"]').querySelector('footer'))) {
            section = 'footer'; detail = 'primary';
        }
    ...
    function getStoredUTM() {
        const utmKeys = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'utm_first_visit'];
        const utmData = {};
    ...
    orderForms.forEach((form) => {
        form.addEventListener('submit', function (e) {
            ...
            let customFields = {};
            switch (formType) {
                case "calculate": title = `Заявка на рассчет стоимости от ${userName}`; break;
                case "basic": title = `Заявка на создание сайта (тариф: Базовый) от ${userName}`; customFields = { UF_CRM_TARIFF: "Базовый" }; break;
                case "optimal": title = `Заявка на создание сайта (тариф: Оптимальный) от ${userName}`; customFields = { UF_CRM_TARIFF: "Оптимальный" }; break;
                case "premium": title = `Заявка на создание сайта (тариф: Премиум) от ${userName}`; customFields = { UF_CRM_TARIFF: "Премиум" }; break;
                default: title = `Заявка на консультацию от ${userName}`;
                    if (contactMethods) customFields = { UF_CRM_CONTACT_METHOD: contactMethods };
                    break;
            }

            const comments = [];
            if (contactMethods && formType === "unknown") comments.push("Предпочитаемый способ связи: " + contactMethods);
            ...
            if (formOverlayEl) {
                ...
                if (submitButton && submitButton.dataset && submitButton.dataset.source) finalSource = normalizeSource(submitButton.dataset.source);
            ...
            fetch(webhookUrl, {
                ...
                .then(data => {
                    if (data && data.result) {
                        ...
                        let contextType = '';
                        if (orderFormContainer && !currentOverlay) contextType = 'order-form';
